...
1. 複製下方清單（或內化為執行流程）。
2. **按順序點擊每個 Raw 連結**，快速瀏覽相關章節（重點看補丁觸發條件、格局定義、卦德凌駕規則、步速匹配、投票制等）。
3. 在分析每匹馬時，對照清單逐一確認。
...
| **1.7** | **確認已檢查值使門、值符星並用於步速預測（階段2.5）** | [sop/sop_pace.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_pace.md)（新建） |
...
| # | 檢查項 | 對應文件（Raw 連結） |
|---|--------|----------------------|
| 10.1 | 確認系統憲法鐵律（十條，含投票制、步速匹配等） | [CONSTITUTION.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/CONSTITUTION.md) |
| 10.2 | 確認本場賽事數據（馬匹、檔位、賠率、近績、晨操等）已完整讀取 | [README.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/README.md) |
| 10.3 | 確認本場使用的奇門版本為陰盤v2.7 + 陽盤輔助 | [sop/sop_qimen.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_qimen.md) 頁首 |
...
若任一檢查項未通過（如未讀取對應文件、未檢查相關補丁、未確認劇本檔位、未執行步速預測、未完成投票制等），則禁止輸出裁決書，必須返回對應階段重新分析。
...
Ver.2.7 2026-06-10 新增步速預測檢查項（5.1-5.3）、投票制與騎師進化檢查項（6.1-6.3）、數據部檢查項（7.1-7.3）；補丁庫擴充至DP-255；憲法鐵律擴充至10條；版本號升級至Ver.2.7
