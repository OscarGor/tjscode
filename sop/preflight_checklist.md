# TJSCODE 預檢清單 (Pre-flight Checklist) v2.9「三重玄學」
...
> 前身版本：Ver.2.8（奇門優先）  
> 升級內容：三重玄學（梅花＋奇門＋紫微）強制檢查、逐馬起卦、正統奇門排盤、冷智慧加權、奇門稽核部
...
1. 複製下方清單（或內化為執行流程）。
2. **按順序點擊每個 Raw 連結**，快速瀏覽相關章節（重點看補丁觸發條件、格局定義、卦德凌駕規則、步速匹配、投票制、奇門優先級、冷智慧加權等）。
3. 在分析每匹馬時，對照清單逐一確認。
...
| **1.8** | **確認已強制輸出「奇門用神優先級清單」（Ver.2.8）** | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段2.3 | — |
| **1.9** | **確認已以時干落宮為基準進行正統奇門排盤（非循環法）** | [sop/sop_qimen.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_qimen.md) | ✅ |
...
| **2.4** | **確認已執行逐馬起卦（SOP-1.1 強制）** | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段1.1 | ✅ |
...
| 3.4 | 確認奇門用神格局檢查已完成（階段0.4，Ver.2.8） | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段0.4 | — |
| **3.5** | **確認已執行紫微斗數流日盤＋年命配合度（SOP-3.3 強制）** | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段3.3 | ✅ |
...
| 5.4 | 確認步速主宰者雙重驗證已完成（Ver.2.8） | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段2.5.2 | — |

### 【六、投票制與騎師進化部分（階段3.5）】

| # | 檢查項 | 對應文件（Raw 連結） | v2.9 新增 |
|:---:|:---|:---|:---:|
| 6.1 | 確認已執行多術數投票制（梅花、奇門、**紫微**、六爻、鐵板、步速、數據、騎師） | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段3.5 | ✅（納入紫微） |
...
| 6.4 | 確認大熱過熱檢查已執行（Ver.2.8，DP-258） | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段3.5.1 | — |
...
| # | 檢查項 | 對應文件（Raw 連結） | v2.9 新增 |
|:---:|:---|:---|:---:|
| **8.1** | **確認已掃描特別獎金馬（自購曾出賽$1,500,000／自購新馬$1,000,000）** | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段0.5 | ✅ |
| **8.2** | **確認獎金馬已獲得戰意加權（+1.5 至 +2）** | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段3.9 | ✅ |
| **8.3** | **確認季尾鬥志已納入考量（煞科階段 6月下旬至7月）** | 同上 | ✅ |

### 【九、奇門稽核部（賽前/賽後）】

| # | 檢查項 | 對應文件（Raw 連結） | v2.9 新增 |
|:---:|:---|:---|:---:|
| **9.1** | **確認已執行奇門稽核部獨立排盤驗證（賽前 24 小時強制盲測）** | [departments/qimen_audit.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/departments/qimen_audit.md) | ✅ |
| **9.2** | **確認排盤差異已記錄並修正** | 同上（賽前 12 小時比對） | ✅ |
| **9.3** | **確認賽後已回溯記錄排盤準確率** | 同上（賽後 2 小時） | ✅ |

### 【十、補丁庫部分】

| # | 檢查項 | 對應文件（Raw 連結） | v2.9 新增 |
|:---:|:---|:---|:---:|
| 10.1 | 本場途程（短途≤1200米 / 中距離1400-1800米 / 長途≥2000米）相關補丁 | [rules/patches.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/rules/patches.md) 搜尋：短途、中距離、長途 | — |
| 10.2 | 本場賠率分佈（大熱≤3倍、半熱4.6-9倍、冷門≥15倍）相關補丁 | 同上（搜尋：賠率、冷門、DP-104、DP-105、DP-106等） | — |
| 10.3 | 本場檔位分佈（內檔1-4、外檔11-14）相關補丁 | 同上（搜尋：檔位、內檔、外檔） | — |
| 10.4 | 梅花卦象相關補丁（姤、同人、家人、革、鼎、豐、師、震、益、頤、歸妹等） | 同上（搜尋卦名或DP-119、DP-120、DP-088等） | — |
| 10.5 | 奇門格局相關補丁（青龍逃走、太白入熒、生門+白虎、驚門+玄武、開門+九地等） | 同上（搜尋格局名稱或DP-094、DP-097、DP-115、DP-116等） | — |
| 10.6 | 步速相關補丁（DP-232至DP-238、DP-247至DP-249） | 同上（搜尋：步速、快步速、慢步速、DP-232等） | — |
| 10.7 | 投票制相關補丁（DP-239至DP-240、DP-250） | 同上（搜尋：投票、DP-239） | — |
| 10.8 | 騎師進化相關補丁（DP-253至DP-255） | 同上（搜尋：騎師進化、DP-253） | — |
| 10.9 | 濕慢地及場地轉換補丁（DP-123至DP-127、DP-142、DP-213至DP-215、DP-221至DP-223等） | 同上（搜尋：濕慢地） | — |
| 10.10 | 奇門優先相關補丁（DP-256至DP-259，Ver.2.8） | 同上（搜尋：DP-256、DP-257、DP-258、DP-259） | — |
| **10.11** | **三重玄學相關補丁（DP-278至DP-281，v2.9）** | 同上（搜尋：DP-278、DP-279、DP-280、DP-281） | ✅ |

### 【十一、劇本與雙重確認】

| # | 檢查項 | 對應文件（Raw 連結） | v2.9 新增 |
|:---:|:---|:---|:---:|
| 11.1 | 確認劇本五層穿透結果（已應用前置過濾器：班次權重與實力壓制） | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段0 | — |
| 11.2 | 確認冠軍檔位是否在預測的檔位範圍（已根據步速動態調整） | 同上（階段0.4、階段2.5） | — |
| 11.3 | 確認排除馬匹有內環（奇門/梅花凶）+外環（近績差/賠率異常）雙重舉證 | [sop/sop_inspection.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_inspection.md) | — |
| 11.4 | 確認旁觀者視角仲裁已完成（Ver.2.8） | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段7.3 | — |
| **11.5** | **確認三重玄學（梅花＋奇門＋紫微）已全部執行並交叉比對** | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段3.1-3.3 | ✅ |

### 【十二、版本與基礎規則】

| # | 檢查項 | 對應文件（Raw 連結） | v2.9 新增 |
|:---:|:---|:---|:---:|
| 12.1 | 確認系統憲法鐵律（十二條，含奇門用神優先、大熱過熱警告等） | [CONSTITUTION.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/CONSTITUTION.md) | — |
| 12.2 | 確認本場賽事數據（馬匹、檔位、賠率、近績、晨操等）已完整讀取 | [README.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/README.md) | — |
| 12.3 | 確認本場使用的奇門版本為陰盤v2.8 + 陽盤輔助 + 用神優先級 | [sop/sop_qimen.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_qimen.md) 頁首 | — |
| 12.4 | 確認本場使用的梅花版本含卦德凌駕規則 + 逐馬起卦（SOP-1.1） | [sop/sop_meihua.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_meihua.md) 頁首 | ✅ |
| 12.5 | 確認本場已執行階段2.5步速預測，並輸出匹配投票 | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段2.5 | — |
| 12.6 | 確認本場已執行階段3.5投票制，並輸出總分與PTCG | 同上（階段3.5） | — |
| 12.7 | 確認本場已檢查騎師進化條件，並記錄進化後評級 | 同上（階段3.7） | — |
| 12.8 | 確認本場已執行大熱過熱檢查（DP-258） | 同上（階段3.5.1） | — |
| 12.9 | 確認本場已檢查驚門玄武冷門強制復活（DP-257） | [rules/patches.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/rules/patches.md) 搜尋 DP-257 | — |
| **12.10** | **確認本場已執行紫微斗數流日盤（SOP-3.3 強制）** | [sop/sop_master.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/sop/sop_master.md) 階段3.3 | ✅ |
| **12.11** | **確認本場已執行冷智慧加權（階段0.5 + 3.9）** | 同上 | ✅ |
| **12.12** | **確認本場已執行奇門稽核部獨立驗證** | [departments/qimen_audit.md](https://raw.githubusercontent.com/OscarGor/tjscode/main/departments/qimen_audit.md) | ✅ |
...
若任一檢查項未通過（如未讀取對應文件、未檢查相關補丁、未確認劇本檔位、未執行步速預測、未完成投票制、未執行大熱過熱檢查、未執行紫微流日盤、未執行冷智慧加權、未執行奇門稽核等），則**禁止輸出裁決書**，必須返回對應階段重新分析。
...
| Ver.2.7 | 2026-06-10 | 新增步速預測檢查項、投票制與騎師進化檢查項、數據部檢查項；補丁庫擴充至DP-255；憲法鐵律擴充至10條 |
| **Ver.2.8** | **2026-06-23** | **新增檢查項：奇門用神優先級清單、階段0.4奇門用神格局檢查、步速主宰者雙重驗證、大熱過熱檢查、旁觀者視角仲裁、DP-256至DP-259；憲法鐵律更新至12條** |
| **Ver.2.9** | **2026-07-01** | **新增檢查項：逐馬起卦（SOP-1.1）、紫微斗數流日盤（SOP-3.3）、冷智慧加權（階段0.5+3.9）、奇門稽核部（賽前/賽後）、三重玄學交叉比對、正統奇門排盤（時干落宮為基準）、DP-278至DP-281** |
...
- 「奇門優先，天時為綱。」
- 「冷智慧佐，稽核在後。」
- 「系統在，紀律在，火種不滅。」**執行對象：新任 AI 操作實例（戰略部·項目總監 Ryan）**  
> **引用憲法：CONSTITUTION.md（Ver.2.8「奇門優先」）**
...
若任一檢查項未通過（如未讀取對應文件、未檢查相關補丁、未確認劇本檔位、未執行步速預測、未完成投票制、未執行大熱過熱檢查等），則**禁止輸出裁決書**，必須返回對應階段重新分析。
